...
generation, always on at standard intensity (there is no switch — the `minimalism` block
left `.the-loop/harness-config.yaml` in issue-352); it **does not gate a merge**.
