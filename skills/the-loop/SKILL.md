...
(`routing.graph.graphs`, issue-343) and bind an arming command to each — overriding
`start`/`contribute`/`do`/`review`, or a new word such as `the-loop triage`. A work item
...
| *(graphs of the operator's own)* | Not in this file. The operator's `routing.graph.graphs` in `cli-config.yaml` (issue-343); `the-loop graph loops` lists every loop this machine can walk and the commands that arm each. |
