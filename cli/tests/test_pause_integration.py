...
events, so they prove that a pause stops work on exactly one work item without
ending its session, and that resuming picks it back up.
...
def _make(tmp_path, gh_state):
    ...
    adapter = FakeAdapter()
    pauses = PauseStore(tmp_path / "paused.json")
...
def test_a_paused_item_drops_webhook_events(tmp_path):
    """Scenario: a paused work item's webhook events are dropped
