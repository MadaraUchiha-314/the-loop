...
- The dashboard SHALL be **three screens** (issue-283, superseding the six-tab
  ...
  surface that replaced them. Since issue-298 the sidebar IS the navigation —
  the design retired the header bar, so the brand block heads the sidebar and
  Events, Settings and the health dot sit in its footer, while Events and
  Settings render as reading columns with a "← Work items" way back — and the
  visual system is **Classical** (`ui/src/styles/classical.css`, vendored from
  the signed-off export under `docs/specs/issue-298/design/`): the loop
  position draws as a tick bar captioned `current · n of m`, with each node's
  name and state kept in the tick's tooltip. A selected session — the outer loop's or a PR
...
|-----------|--------------|-------|
| issue-298 | The dashboard's design overhaul: the Industry system is replaced wholesale by **Classical** (Cormorant Garamond over Lora, hairline rules, color as stroke), vendored as `ui/src/styles/classical.css` from the owner's signed-off export (checked in under `docs/specs/issue-298/design/`). The header bar is retired — the Work sidebar carries the brand, the Events/Settings links and the health dot — the labelled node rail becomes a tick bar captioned `current · n of m` (node names in tooltips), the transcript reads as an editorial thread ("You" / "the-loop" speakers, folds for thinking and tool calls), and Events/Settings render as reading columns behind "← Work items". Presentation only: every API connector, route, model join and control verb is unchanged | [design](../specs/issue-298/design/), [issue](https://github.com/MadaraUchiha-314/the-loop/issues/298) |
