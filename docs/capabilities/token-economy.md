...
|-----------|--------------|-------|
| issue-365 | The execution log was retired as a token lever in its own right (2026-09-14): a 130-line template per work item, a checkpoint appended at every node boundary and a prose entry demanded before every context reset, all re-deriving what the transcript, the commits and `graph-state.json` already held. State worth offloading to disk is state something reads | [spec](../specs/issue-365/), [decision-126](../decisions/decision-126.md), [spec-workflow](spec-workflow.md), [issue](https://github.com/MadaraUchiha-314/the-loop/issues/365) |
