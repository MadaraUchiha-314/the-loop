...
- `the-loop service start|stop|status` and `the-loop ui dev|build` SHALL manage the
  control-plane API service and UI (issue-161); core-capability commands SHALL
  execute through that service as their only mode (`check` and `events` route
  today; the rest are being switched — see
...
|-----------|--------------|-------|
| issue-161 | Re-layered as core → API → clients: `the_loop.core` facade, the control-plane service (`service start\|stop\|status`, `[service]` extra), service-routed `check`/`events`, `ui dev\|build`, the `/mcp` endpoint and the `ui/` frontend | [spec](../specs/issue-161/), [decision-058](../decisions/decision-058.md), [control-plane](control-plane.md), [issue](https://github.com/MadaraUchiha-314/the-loop/issues/161) |
