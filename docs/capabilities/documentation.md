...
- The site, the root `README.md`, the CLI's PyPI page and the dashboard SHALL carry
  the-loop's mark — the Ensō chosen in issue-398 — from **one generated source**:
  `docs/specs/issue-398/design/generate.py` writes `docs/assets/the-loop-logo{,-light,-dark}.svg`,
...
|-----------|--------------|-------|
| issue-398 | the-loop's mark adopted across the user-facing surfaces: the root README (a theme-aware `<picture>`), the site's nav logo, hero image and favicon (`docs/public/`), the CLI's PyPI README, the dashboard's favicon, and the Slack app icon (a 1024 px raster plus the manual upload step in the Slack guide) — every file generated by `docs/specs/issue-398/design/generate.py` and pinned by its `--check` | [spec](../specs/issue-398/), [issue](https://github.com/MadaraUchiha-314/the-loop/issues/398) |
