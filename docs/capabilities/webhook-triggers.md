...
- WHEN no session matches THEN the router SHALL spawn a new session per
  ...
  auto-execute labels) using the configured prompt templates — **and, since
  issue-106, only once an authorized user has explicitly started the work item** (see
  *Execution control* below).
- **The arming gate is a set of labels, every one required** (issue-381,
  [decision-131](../decisions/decision-131.md)). `routing.autoExecuteLabels` is a list
  (default `["the-loop: auto-execute"]`, at least one entry); WHEN an issue or PR carries
  **every** label in it THEN the item SHALL count as labelled — on a `labeled` action the
  label being added is read together with the item's label set, so the last missing label
...
- **Which instance an event belongs to is decided before anything else acts on it**
  (issue-322, [decision-110](../decisions/decision-110.md)). Several instances of
  the-loop may watch one repository; each answers *is this mine?* from its own
  `instance` block — see [instances](instances.md) for the modes, the address token and
  the managed set. The seam sits in `Dispatcher.handle` after linkage verification and
  the control parse and before authorization, recording and matching, and its only power
  is refusal: an event outside the instance's scope is a settled `dispatch.dropped` (or a
  `control.rejected` for an authorized keyword) with the reason, and no reaction,
  comment, record or session write. An unnamed, open instance — every config without the
  block — reaches this bullet's successors exactly as before.
- **Execution control: the label is necessary, not sufficient** (issue-106,
  ...
  *who* may be an input; the declared keywords say *when*:
  ...
  that the item runs without one — `the-loop review` (issue-279), the shape a third
  time: it selects the **review loop**, the-loop as the reviewer of a change, and —
...
- **Work-item collaborators: input for one work item, and nothing more** (issue-307,
  ...
  *project's* stewards and their roles for the plugin and is never read by the daemon.)
  ...
  - **A roster entry may carry a Slack member id** beside, or instead of, the login
    (issue-389, [decision-133](../decisions/decision-133.md)): `the-loop
    add-collaborator slack:U…` on the ticket, `@the-loop add-collaborator @member` by
    mention in the work item's Slack room, or `--slack <id|@handle>` on the CLI (a
    handle resolved through the Slack directory and refused when it resolves to none or
    several). At least one of the two ids is required; an entry with neither, or a
    `slack` value that is not a member id, authorizes nobody. `remove-collaborator`
    revokes by either id. WHEN a collaborator is known by Slack id THEN their **mentions
    on Slack** SHALL be input on that work item — `record-context`, a reply, `help` —
    and nothing that binds it (no `record-decision`, no keyword, no `--listen`); a
    Slack-id-only collaborator is input on Slack only, because the ledger's ingress
    still reads logins and has none for them. Their records name them by the ids the
    roster holds, never by what the message says.
...
- **That routing decision is recorded, not recomputed — and each PR is a session of its
  ...
  - **Deliberately out of scope here:** a PR endpoint has no process graph. The graph
    ...
    sets).
- The auto-execute labels SHALL work on **PRs directly**: a labelled PR linked to no
...
- WHEN a work item **ends** — an `issues` event with action `closed`, or a
  ...
  **auto-closed** rather than resumed: registry entry closed, tmux session handled per
  ...
  `pr-closed`) and `merged` (a merge, or a pull request recorded on the work item that
  merged — issue-405). A closure that reaches a live session standing on its graph's
  **terminal node** is **held** first (issue-405 P2, `session.closing`) until the
  session claims completion or `routing.tmux.finishGraceSeconds` runs out — see
  [interactive-sessions](interactive-sessions.md). A close event SHALL never spawn a
  session, whatever `spawnOnUnmatched`
...
- **A record carrying only `poll` is asked lazily** (issue-332,
  ...
  sixty cycles' worth of `polling.intervalSeconds` (the later of **this machine's**
  `lastPolledAt` and `closureCheckedAt` for it, kept in `<state.root>/local/poll-clocks.json`
  since issue-382; an absent or unparsable stamp counts as due) THEN the next
...
| issue-381 | The arming label became a **list, every entry required** (2026-09-18): `routing.autoExecuteLabels` replaces `autoExecuteLabel`, and a github poll source's `labels` replaces its `label`, so an operator sharing a repository adds a label of their own to the shared one and instances stop arming each other's items. The router, the dispatcher's arming check, the poller's `gh` listing (one `--label` per label, the listing re-checked provider-side) and its presence event all read the list through one all-of predicate; an empty list arms nothing. Breaking: config version `0.10.0`, the old keys are refused rather than read and `the-loop migrate-config` wraps them | [spec](../specs/issue-381/), [decision-131](../decisions/decision-131.md), [routing](../config/cli/routing-options.md), [issue](https://github.com/MadaraUchiha-314/the-loop/issues/381) |
