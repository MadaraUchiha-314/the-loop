...
|--------------|-----------------|-----|
| D1 | A new `channels` CLI-config section and `the_loop/channels/` package — **not** an extension of `integrations.slack` | An integration is a transport for one call; a channel adds an event filter, verbosity, an inbound allow-list and conversation state. One schema shape cannot carry both without every key turning conditional. The webhook notification keeps working unchanged. |
...
**Costs, accepted.** The graph's `notify` hook still posts through the incoming
webhook — two Slack configurations can coexist and the docs must keep them distinct.
Per-collaborator targeting (`collaborators[].notifications.channels`) remains
