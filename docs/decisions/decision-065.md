...
| **D8 — the graph assigns, not just judges** | `deliver-assignment` on every non-terminal agent node's entry chain: entering a node renders that node's assignment (where the item stands, what to produce, the exact claim command) and pushes it into the loop's bound session through a dispatcher-provided channel | The owner's "it should always be the-loop's graph that's running the show" (PR #173). Every verdict was already the graph's; this makes the *initiative* the graph's too: graph assigns → session works → session claims → graph verdicts → graph assigns the next node. The channel is daemon-injected; on the CLI path the hook skips, because the claiming session already reads the same facts from its command's JSON envelope — pushing there would deliver every assignment twice. Best-effort: a failed push is recorded (`graph.assignment_failed`) and never gates the node, since the same state is durable and re-rendered into every event prompt. |
