...
| # | Title | Status | Date |
|---|-------|--------|------|
| [136](decision-136.md) | An operator declares graphs of their own in the CLI config (top-level `graphs`), never a repository; `routing.control.commands` binds an arming command to one by name — an overridden `start`/`contribute`/`do`/`review`, or a new word that is `start` with a loop recorded on the control record; selection fails closed to the declaration; a custom graph keeps the shipped compiler, hooks and phase vocabulary; `pdlc-` is reserved; attachments may be scoped to loops | proposed | 2026-09-23 |
| [135](decision-135.md) | A file a person attaches is fetched and forwarded, never interpreted: the daemon downloads it to `<state.root>/local/attachments/<slug>/` with a host-checked, capped fetch and hands the session the path; a voice note is Slack's own transcript, verbatim; the ticket keeps 📎 names and links, never a path; no speech-to-text, OCR or model call on a file anywhere in the loop | proposed | 2026-09-21 |
| [134](decision-134.md) | The Slack room rework is a delivery policy (`RoomPolicy.decide`) over an unchanged GitHub ledger — collapse, suppress, edit-in-place and thread for a work item's own room, in a first-person voice behind `room.style: agentic`; a human gate parks when it publishes its request so the first answer counts (B8); a spawned session inherits the daemon's config path so its events reach the bus (B6) | proposed | 2026-09-19 |
| [133](decision-133.md) | A Slack conversation addresses the-loop by mention (`app_mention`, never text matching; a DM and an authorized-only `all` room excepted); a fixed grammar with natural language as the fallthrough; `record-context` and `record-decision` are marked, enveloped records delivered by the channel with a preset frame; grants, not subscriptions; `context.md` and the existing decision log are the session's auditable view; two tiers of speaker with collaborators known by Slack id; a shortcut is the typed mention | proposed | 2026-09-19 |
| [132](decision-132.md) | A poll clock is a machine reading, not a fact about the work: `lastPolledAt` and `closureCheckedAt` move to `<state.root>/local/poll-clocks.json`, the split happens at the poller's storage boundary, a stale record is read once and then stripped, and the record is written only when it changed | proposed | 2026-09-18 |
...
| [119](decision-119.md) | `state.root` resolved once at the load boundary, anchored on the config file; spawns carry the config path | proposed | 2026-09-11 |
...
| [114](decision-114.md) | A poll source's bare `OWNER/REPO` inherits the resolved GitHub host when the source is built, not when a ref is compared | proposed | 2026-09-09 |
| [113](decision-113.md) | A closed work item is stamped `ended` on its portable record and demoted, never deleted; `graph` stays | proposed | 2026-09-09 |
