...
| # | Title | Status | Date |
|---|-------|--------|------|
| [136](decision-136.md) | An operator declares graphs of their own in the CLI config (`routing.graph.graphs`), never a repository; an arming command selects one by name — an overridden `start`/`contribute`/`do`/`review`, or a new word that is `start` with a loop recorded on the control record; selection fails closed to the declaration; a custom graph keeps the shipped compiler, hooks and phase vocabulary; `pdlc-` is reserved; attachments may be scoped to loops | proposed | 2026-09-23 |
