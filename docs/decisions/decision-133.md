...
| D6 | **A pull request that changes the pipeline rehearses the no-model half; every other pull request runs nothing.** | The workflow cannot otherwise be exercised before it reaches `main`. Running graphify on every pull request would add forty seconds to each and prove nothing new. The rehearsal has its own concurrency group, so it can never displace a pending `main` rebuild (GitHub keeps one pending run per group). |
| D8 | **The write token is on disk for the commit step only.** Both checkouts run with `persist-credentials: false`; the commit step writes the token into the remote URL after graphify has exited. | graphify is a third-party tool running with the repository's own key in its environment; a compromised release must not also be able to push to `main`. Raised by the graphify-labs review on PR #386. |
