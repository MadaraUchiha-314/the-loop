...
iteration happens on a **durable, reviewable** surface — the pull request **or the
ticket** — never in a terminal. Which of the two the outer loop uses is
`workflow.outerLoop.surface`; the inner loop still has no choice. The configuration this
paragraph refused — specs reviewed where the reasoning dies with the scrollback — is
still refused.)* It
