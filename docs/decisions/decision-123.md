...
| D13 | **What the agent can infer, it infers; what the operator runs, the operator sets.** `repository`, `tooling`, `hooks` and `observability` leave the harness config: the skill reads the repository's layout, tooling and git hooks off the repository itself at the start of every work item (the detection table that used to feed `/init`), and a project's log levels are its own logging configuration. `reviews` moves to the CLI config (top-level, four defaulted keys) and `the-loop critic policy` prints it. | The owner's third review: *"this can be inferred from repo. we can add it to the-loop's skill to infer this. remove from config"*, *"can be inferred from repo assets like package.json or pyproject.toml"*, *"remove this."*, and for `reviews`: *"this should be in cli-config not harness config."* A declared fact that a manifest also states can only disagree with the manifest; the operator who runs the review rounds is the one who knows how many their machine can afford. |
| D14 | **The doc trees are a convention, not a setting.** `workflow` leaves the harness config: work-item specs live at `docs/specs/<id>/`, capability docs at `docs/capabilities/`, learnings at `docs/learnings/`. The CLI keeps `routing.graph.specDir` and `--spec-dir` for an instance whose repositories depart from it. Five keys remain: `version`, `customInstructions`, `testing`, `apiSpecs`, `design`. | The owner's fourth ruling: *"remove workflow."* A location every repository is expected to share is a convention; a key that lets each repository choose is a way for two tools to disagree about where the record is. |
...
- **Five keys remain** — `version`, `customInstructions`, `testing`, `apiSpecs`,
  `design`: the facts about the repository the agent cannot read off the repository
  itself. The doc trees are the loop's convention (`docs/specs/<id>/`,
  `docs/capabilities/`, `docs/learnings/`). Everything that was a policy switch is now a sentence in a
