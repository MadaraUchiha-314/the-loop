...
the-loop standing start [NAME]      # omit NAME for every declared session
...
Declare them under
[`standingSessions`](/config/cli/standing-sessions-options) — the block is **off by
default**, and an entry names harness arguments and a working directory, so review one
like code.
...
Omitting the name means *every declared session* for `start` and *every recorded one* for
`stop` — which is what makes a session you disabled after starting it still stoppable, and
what keeps a declared-but-never-started entry out of `stop`'s way. `restart` needs a name.
