...
| T14 | ✅ *(added, PR #229 review round 2 / issue-231)* Single-process mode | unit: 5 hosted cases in `tests/test_core_lifecycle.py` (host skips spawn + awaits locks; `hostIngresses: false` keeps standalone; disabled service never hosts; hosted fail when the service is down; stop-via-service with real `RunLock`s); integration: `tests/test_hosted_ingress_integration.py` — real `the-loop start` boots one pid holding all three locks, hosted receiver answers `/health` on its port, `status --format json` rows say `hosted: true`, `stop` releases everything; a standalone daemon's lock is skipped, not stolen; then the full suite | unit 15 passed; integration 2 passed; full suite 2041 passed, 1 skipped |
