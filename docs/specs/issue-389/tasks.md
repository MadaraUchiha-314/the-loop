...
- [ ] **1. Catalog rows, schema text, docs pins.** `channels/events.py` gains
...
- [ ] **2. Manifest and probe.** `slack-app-manifest.yaml` gains `app_mentions:read`, the
...
- [ ] **3. `channels/verbs.py`.** `strip_mention`, `parse_verb`, `compose_keyword`,
  `help_text`. _Req:_ R3.1, R3.2, R3.4 (design §3). _Test:_ T1 (`test_channels_verbs.py`).
- [ ] **4. Roster: a Slack id.** `CollaboratorRecord.slack`, `login` optional when
...
- [ ] **5. Room: the listen mode.** `CollaborationChannel.listen` with the two-value
  ...
  T8 A12, T10.
- [ ] **6. Ledger record shapes.** `GitHubLedger.record` mirrors `context.added` and
...
- [ ] **7. Session frames.** `reply_session(kind=)` with the `context` and `decision`
...
- [ ] **8. Inbound: the address, the tiers, the verbs, the acts.**
  ...
  T8 A1–A10.
- [ ] **9. Listener routing, shortcuts, modal.** `handle` routes `app_mention`,
...
- [ ] **10. `the-loop channels records`.** A read-only verb listing a work item's
...
- [ ] **11. Status lines and the poll skip.** `channels status` prints `mentions:`,
