...
- [x] T1 — `cd cli && uv run python -m pytest -q tests/test_channels_mentions.py tests/test_channels_verbs.py tests/test_collaborators.py tests/test_workchannels.py tests/test_channels.py`
- [x] T2 — `cd cli && uv run python -m pytest -q tests/test_channels_mentions_integration.py tests/test_channels_shortcuts_integration.py tests/test_collaborators_cli.py tests/test_workchannels_cli.py tests/test_channels_records_integration.py`
- [x] T6 — `cd cli && uv run python -m pytest -q tests/test_channels_commands.py tests/test_channels_dm.py tests/test_channels_shortcuts.py`
- [x] T8 — `cd cli && uv run python -m pytest -q tests/test_channels_mentions_security.py`
- [x] T10 — `cd cli && uv run python -m pytest -q tests/test_channels_upgrade.py tests/test_config_schema_parity.py tests/test_docs_parity.py tests/test_graph_parity.py tests/test_bus.py`
- [ ] T11 — the manual procedure in `evidence/manual.md`, against the workspace named by the credentials above
- [x] T12 — `make check`

## Verification results

Executed 2026-09-19 at the PR's head, in a cloud checkout of this repository (`cd cli
&& uv sync`); T11 needs a Slack workspace and was not run here.
