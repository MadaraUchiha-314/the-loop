...
|----|---------------|--------|
| [#129](https://github.com/MadaraUchiha-314/the-loop/pull/129) | spec + T1–T8 | open |
...
**Real git agrees with the recipe** (`git check-ignore -v`, against files created under
`.the-loop/` and then removed):

| Path | Verdict | Matched by |
|---|---|---|
| `.the-loop/sessions/github-octo-repo-15.json` | ignored | `.the-loop/sessions/*` |
| `.the-loop/sessions/poll-state.json` | **tracked** | `!.the-loop/sessions/poll-state.json` |
| `.the-loop/sessions/control/github-octo-repo-15.json` | **tracked** | (no pattern matches) |
| `.the-loop/sessions/control/tmp9k2f.tmp` | ignored | `.the-loop/sessions/control/*.tmp` |
| `.the-loop/logs/events.jsonl` | ignored | `.the-loop/logs/` |
| `.the-loop/gh-webhook.pid` | ignored | `.the-loop/*.pid` |

`git status --porcelain .the-loop` then showed exactly `?? .the-loop/sessions/` — the two
portable files offered for tracking, the three local ones invisible.
