...
- [x] **1 · Declare the path.** Add `StateLayout.channels_outbox`
...
- [x] **2 · The outbox.** Write `cli/the_loop/channels/outbox.py` — `path_for`,
...
- [x] **3 · The drain.** Add `drain` and `start_drainer` to the same module: the per-entry
...
- [x] **4 · The bus writes it.** Add the three-guard call at the end of
...
- [x] **5 · The daemons drain it.** Start `outbox.start_drainer` beside
...
- [x] **6 · The status line.** Add `channelDelivery` to `lifecycle.status_all`'s report
...
- [x] **7 · The session is told.** In `core/sessions.py`'s `ask_session`, add the
...
- [x] **8 · The scenarios.** Write `cli/tests/test_channels_outbox_integration.py` with
      Gherkin docstrings: the report's full ask → queue → recover → deliver walk, and the
      `status` surface.
      _Requirements:_ all.
      _Test:_ testing-plan T4, T5.

- [x] **9 · The capability docs.** Update `docs/capabilities/channels.md` (the outbox, the
...
- [x] **10 · Verify and record.** Run the matrix and write `evidence/verification.md`,
