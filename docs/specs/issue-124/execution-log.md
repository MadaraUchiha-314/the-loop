...
| 4 | critic | — | **Not run: `reviews.critics: []`** — no critic harness is configured in this repository, so `the-loop critic run` has nothing to invoke. Recorded rather than silently skipped | `.the-loop/harness-config.yaml` |
