...
- **Did:**
  ...
  - Confirmed the completion-signal question from the harnesses' documented behaviour:
    ...
    the question moot by exiting with a terminal result object. Cursor's CLI hook coverage
    is reported as partial — flagged as an open question to verify, not designed on.
