...
- [ ] **1 · The compiler's new rules.** `PHASE_VOCABULARY`, the `Node.command` grammar,
...
- [ ] **2 · The catalog.** `cli/the_loop/graph/catalog.py` — `CustomGraph`, `Catalog`,
...
- [ ] **3 · Loading and resolving.** `load_graph(..., catalog=)`, the `x-` resolution,
...
- [ ] **4 · Scoped attachments.** `Attachment.loops`, its parse and validation, `apply`'s
      skip, the digest.
      _Requirements:_ R6.1, R6.2.
      _Test:_ testing-plan T6.

- [ ] **5 · The runtime builder.** `build_runtime` honours declared names and `guest`;
      `core/graphs._recorded_loop` resolves against the catalog.
      _Requirements:_ R4.2, R4.3, R5.1, R5.2.
      _Test:_ testing-plan T3.

- [ ] **6 · The control vocabulary.** `ControlConfig.bindings/loops`, `from_mapping(…,
...
- [ ] **7 · The daemon path.** The dispatcher records the loop and logs it; every
...
- [ ] **8 · `the-loop graph loops`.** Text and JSON; compile without importing modules;
      exit 1 on a failure.
      _Requirements:_ R7.1–R7.3.
      _Test:_ testing-plan T7.

- [ ] **9 · Integration scenarios.** `cli/tests/test_custom_graph_integration.py` with
      Gherkin docstrings.
      _Requirements:_ R3.5, R3.6, R4.1; abuse case 3.
      _Test:_ testing-plan T5, T8.

- [ ] **10 · Schema and samples.** Both `cli-config.schema.json` copies; the commented
...
- [ ] **11 · Docs and decision.** `docs/capabilities/process-graph.md` (+ `cli.md` for the
...
- [ ] **12 · Verify and record.** Run the plan, write `evidence/`, tick this list.
