...
| T5 | UI / visual | n/a — the-loop has no product UI (`design.uiArtifacts.format: html`, none authored). The documentation site is rendered markdown, checked by T6 rather than by screenshot | | |
...
reason there. One matrix row was **replanned during implementation** rather than skipped:
T2 was planned `n/a` and turned out to have real coverage, so it was promoted to `yes`,
executed, and the reasoning that got it wrong is recorded in the matrix rather than edited
out.
