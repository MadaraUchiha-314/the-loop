...
| T12 | Unit | yes | `Session` round-trips `channels.slack.cursors`, absent rather than empty; every registry fixture written before the change round-trips byte-identically (R5.1, R8.1) | `uv run --project cli python -m pytest -q cli/tests/test_state.py -k registry` |
