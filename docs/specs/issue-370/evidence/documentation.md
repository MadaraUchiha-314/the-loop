...
| `process-graph.md` (2nd pass) | A pull request armed as its own work item is recorded in `pullRequests[]` with `self: true` and no `stateDir`; the "is it a PR?" question is read from the arming event | yes (issue-370 owner ruling) |
...
|----------|--------------|
| `docs/cli/state.md` (2nd pass) | What a `self: true` row means and why it carries no `stateDir` |
...
| `docs/decisions/decision-129.md` (+ index) | New: routing may guess, tracking may not — plus the owner's ruling that the rule is uniform across what a work item *is*, and why loop membership is not the signal |
