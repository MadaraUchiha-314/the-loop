...
checkmarks: pending           # pending | complete
overrides: {}
---
...
| T2 | Integration (scenario) | yes | end-to-end through the webhook receiver: `always` spawns a second tmux session for a **same-repository** pull request when it gets a clone of its own; the same config **declines** to one session under `strategy: worktree` | `make test` (`cli/tests/test_webhook_routing_integration.py`) |
...
- **Repositories:** this repository only. The integration rows drive the receiver in-process
  with a faked harness adapter and a faked tmux, exactly as the existing routing integration
  tests do; the workspace rows use real `git` against local repositories created in a
  `tmp_path`.
- **Services / containers:** none. No network, no GitHub, no `gh`.
- **Fixtures & data:** the existing `server_factory` / `TmuxConfig` fixtures in
  `cli/tests/`; a local `git init --bare` origin for the workspace rows.
...
|----------|--------------------|---------|----------|
| Red run | `pytest -q` on the new tests, before the implementation | 13 failed, as designed — the two red roots plus their dependants | [`evidence/red.md`](evidence/red.md) |
...
| T2 | `pytest -q cli/tests/test_webhook_routing_integration.py` | pass | [`evidence/unit-and-integration.md`](evidence/unit-and-integration.md) |
