...
| T2 | Unit | yes | `critics.load_critics(path)` reads `critics[]` from a CLI config: absent → `[]`, duplicates refused, `find_critic(name, path)` names the alternatives; the command surface with `--config` | `uv run --project cli python -m pytest -q cli/tests/test_critics.py` |
| T3 | Unit | yes | `core.repo`: `scenarios` uses the caller's globs or the defaults; `instructions` takes `docs` (string or mapping) and `on_missing`; `critics` lists the CLI config's entries and **ignores a committed `reviews.critics[]`** (A1) | `uv run --project cli python -m pytest -q cli/tests/test_core_repo.py cli/tests/test_cli.py` |
...
| T11 | Static | yes | no module under `cli/the_loop/` names `harness_config`, `harness-config.default`, `repoInitialized`, `allow_repo_hooks` or `.adopt(`; ruff, ruff format, pyright, markdownlint | `make check` plus a `grep` recorded in the evidence |
