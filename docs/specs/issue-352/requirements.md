...
A third review (07:43Z) ruled on what was left: `repository` and `tooling` — *"can be
inferred from repo. we can add it to the-loop's skill to infer this. remove from
...
6. WHEN a document, command or docstring names one of those keys as configuration,
   ...
   not exist. (AC3.6)
7. THE schema, the template and this repository's config SHALL also drop `repository`,
