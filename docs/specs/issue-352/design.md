...
| D10 | Schemas: harness schema loses nine keys in the first pass and nine policy blocks in the second (`autonomy`, `security`, `tdd`, `minimalism`, `tokenEconomy`, `selfImprovement`, `contextManagement`, `userInteraction`, `externalTools`; the `$defs` only they used go with them) and rewrites its onboarding groups (`people` keeps no key — it establishes `collaborators.yaml`; `automation` is gone); CLI schema gains `critics` (the old items, minus object examples the runtime validator cannot walk, `timeoutSeconds.minimum: 1`) and `routing.graph.hooks`, loses `repoHooks`, defaults `specDir` to `docs/specs`; `cli/the_loop/schemas/` copies follow. `migrations.CURRENT_CONFIG_VERSION = "0.9.0"`; `_retire_repo_hooks` pops the key and notes where critics and hooks live. | `.the-loop/*.schema.json`, `cli/the_loop/schemas/`, `migrations.py` |
| D11 | Configs/templates: harness `0.3.0` without the removed blocks (eleven top-level keys remain: `version`, `repository`, `workflow`, `tooling`, `customInstructions`, `testing`, `apiSpecs`, `design`, `hooks`, `observability`, `reviews`); CLI `0.9.0` with `critics: []` and `routing.graph.{specDir, hooks}` commented; `validate_config.py` drops the packaged-default target; `hooks.json` and `rules/the-loop.mdc` both test for `harness-config.yaml`. | `.the-loop/`, `skills/the-loop/templates/`, `scripts/`, `hooks/`, `rules/` |
| D12 | Skill and commands: `SKILL.md` § Configuration rewritten with the key → flag table and the surviving key list; `security.md`, `token-economy.md`, `context.md`, `minimalism.md` state their rules directly instead of as config keys (the second review); `test_writing_parity` names the writing skill as a constant instead of reading `userInteraction.writingStyle.skill`; `reviewing.md`, `automation.md`, `collaboration.md`, `workflow.md`, `tooling.md`, `instructions.md`, `testing.md`; `init.md` step 4 from the graph, `upgrade-the-loop.md` migration block, `<phaseLabelPrefix>` → `loop:` everywhere. | `skills/`, `commands/` |
| D14 | The third review: `repository`, `tooling`, `hooks`, `observability` leave the harness schema (six keys remain, `required: [version]`; onboarding groups `instructions`, `workflow`, `testing`, `people`, `contracts-design`); `reviews` becomes the CLI schema's top-level `reviews` (four defaulted keys). `critics.load_review_policy(config_path)` reads it the way `load_critics` does (absent → defaults, malformed → `CriticConfigError`); `core.repo.review_policy()`, `the-loop critic policy [--format json]`, `GET /repo/critics/policy` (`repoReviewPolicy`, in the authored contract), `Repo.review_policy` (SDK, MCP). `/init` proposes only the six keys' values; `reference/tooling.md`'s detection is the agent's per-session procedure. | `critics.py`, `core/repo.py`, `commands/critic_cmd.py`, `api/routes.py`, `api/mcp.py`, `sdk/client.py`, `docs/api-specs/openapi/the-loop.v1.yaml`, schemas, `commands/init.md` |
| D15 | The fourth review: `workflow` leaves the harness schema (five keys remain; onboarding groups `instructions`, `testing`, `people`, `contracts-design`). The doc trees are the loop's convention — `docs/specs/<id>/`, `docs/capabilities/`, `docs/learnings/` — written as literal paths in the skill, commands and docs; `routing.graph.specDir`'s description says the convention and when to depart from it; `/init` scaffolds the fixed trees; `upgrade-the-loop` moves a differently placed tree with the operator's confirmation or reports it. | schemas, configs, `commands/init.md`, `commands/upgrade-the-loop.md`, `skills/`, `docs/` |
