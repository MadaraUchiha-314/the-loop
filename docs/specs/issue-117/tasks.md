...
- [ ] 1. **The parity test, red.** `cli/tests/test_docs_parity.py` with P1–P4 from
  ...
  - _Test:_ `uv run --project cli python -m pytest cli/tests/test_docs_parity.py`
    — **red**: 4 failures (no `docs/cli/commands/`, no `docs/config/cli/`)

- [ ] 2. **Config section landing + harness config.** `docs/config/index.md` (the two
  ...
  - _Test:_ page set on disk; `make lint` (markdownlint) green

- [ ] 3. **The six CLI config pages** — `docs/config/cli/{index,webhook-options,routing-options,polling-options,integrations-options,observability-options}.md`.
      ...
      **Type** and **Default** taken from `.the-loop/cli-config.schema.json`. Covers all
      81 leaves, which necessarily includes the four undocumented blocks and excludes the
      removed `ghBinary`. Carries the three load-bearing security statements (design
      §Security design).
  - _Depends on:_ 2
  - _Requirements:_ R3.2, R3.3, R3.4, R4.1, R4.2, R4.3
  - _Test:_ P3 **and** P4 in `test_docs_parity.py` go **green** (red→green)

- [ ] 4. **CLI section core pages** — `docs/cli/{index,installation,getting-started,concepts}.md`.
  ...
  - _Depends on:_ 1
  - _Requirements:_ R1.2, R1.3, R1.4, R1.5
  - _Test:_ pages on disk; markdownlint green; getting-started YAML validated against
    `.the-loop/cli-config.schema.json` with `scripts/validate_config.py`-style check

- [ ] 5. **Command pages** — `docs/cli/commands/index.md` (the one table) plus one page
  ...
  - _Depends on:_ 4
  - _Requirements:_ R2.1, R2.2, R2.3, R2.4, R2.5, R4.4
  - _Test:_ P1 **and** P2 in `test_docs_parity.py` go **green** (red→green)

- [ ] 6. **`docs/cli/extending.md`** — the `Command`/`@register` contract, and the fact
      that adding a command now also requires its page (P1).
  - _Depends on:_ 5
  - _Requirements:_ R2.1
  - _Test:_ markdownlint green

- [ ] 7. **`cli/README.md` → package landing page**, `docs/scripts/sync-content.mts`
  ...
  - _Depends on:_ 3, 5
  - _Requirements:_ R5.1, R5.2, R5.3, R5.4
  - _Test:_ `bun run docs:sync` produces no `docs/cli.md`; grep finds no relative site
    link in `cli/README.md`; full suite still green

- [ ] 8. **Navigation** — `docs/.vitepress/config.mts`: `CLI` and `Config` in `nav`,
      sidebar trees for `/cli/` and `/config/`, `configuration` dropped from the
      Reference sidebar.
  - _Depends on:_ 3, 5, 6
  - _Requirements:_ R1.1, R3.1, R6.5
  - _Test:_ `bun run docs:build` green; nav/sidebar render in the built output

- [ ] 9. **Link hygiene across the repo** — root `README.md`, `docs/index.md` (CLI as a
  ...
  - _Test:_ `grep -rn '](/cli)\|](/cli#\|/reference/configuration'` returns nothing
    outside `docs/specs/**`

- [ ] 10. **Capability docs** — mint `docs/capabilities/documentation.md` (the docs site
  ...
  - _Depends on:_ 9
  - _Requirements:_ R6.1 (and the ready-to-ship capability-docs gate)
  - _Test:_ markdownlint green; every link in the new doc resolves

- [ ] 11. **Gates.** `make lint`, `make typecheck`, `make validate`, `make test`,
  ...
  - _Depends on:_ 10
  - _Requirements:_ R6.4, and the security-review gate
  - _Test:_ all five commands green; checklist recorded in the execution log

- [ ] 12. **Evidence + review.** Execution log: red→green transitions, the C5 fidelity
  ...
  - _Depends on:_ 11
  - _Requirements:_ all
  - _Test:_ `the-loop check issue-117` (the loop's own gate) reports no unmet reachable node
